...
<style>
  ...
  .contenedor {
    ...
    gap: 10px; /* Espacio pequeño entre juegos */
    max-width: 1020px;
    margin: auto;
  }
  ...
  .svg-container {
    width: 100%; /* Asegurar que cada SVG ocupe todo el ancho disponible */
    border: 1px solid red; /* Borde temporal para verificar si aparece */
  }

  .svg-container svg {
    width: 100%; /* Asegurar que los SVG se escalen */
    height: auto;
    display: block;
  }
</style>
...
<script>
  ...
  async function loadAndModifySVG(url, juego) {
    ...
    console.log("SVG Modificado:", svgElement.outerHTML); // Verificación en consola
    return svgElement.outerHTML;
  }
