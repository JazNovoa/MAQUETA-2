    ...
    try {
      juegosSVG = await Promise.all(
        juegos.map(async (juego) => {
          ...
          // Cambia el color del SVG según la adictividad
          // Aplica el color a todos los elementos path dentro del SVG
          const updatedSVGContent = svgContent.replace(/<path/g, `<path style="fill:${adictividad(juego.Adictividad)};"`);
