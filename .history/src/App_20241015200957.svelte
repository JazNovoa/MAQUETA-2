...
<script>
  ...
  juegosSVG = await Promise.all(
    juegos.map(async (juego) => {
      ...
      // Añade el color de trazo según el continente
      const svgWithStroke = svgContent.replace(
        /<rect/g,
        `<rect stroke="${continente(juego.Continente)}" stroke-width="2"` // Ajusta el ancho del trazo según sea necesario
      );
      
      return { ...juego, svgContent: svgWithStroke };
    })
  );
})();
